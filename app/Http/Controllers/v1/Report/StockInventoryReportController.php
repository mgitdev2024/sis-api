...
class StockInventoryReportController extends Controller
{
    ...
    public function onGenerateDailyMovementReport(Request $request)
    {
        ...
        try {
            ...
            return $this->dataResponse('success', 200, 'Report generated successfully.', $reportData);
        } catch (Exception $exception) {
            return $this->dataResponse('error', 400, 'Report generated unsuccessfully.', $exception->getMessage());
        }
    }
    ...
    private function getBulkBeginningStock($transactionDate, $itemCodes, $storeCodes, $storeSubUnitShortName)
    {
        ...
        // Bulk query for stock inventory counts with optimized indexes
        $query = DB::table('stock_inventory_count as sic')
            ->join('stock_inventory_items_count as siic', 'sic.id', '=', 'siic.stock_inventory_count_id')
            ->where('sic.created_at', '>=', $subTractedDate . ' 00:00:00')
            ->where('sic.created_at', '<=', $subTractedDate . ' 23:59:59')
    ...
    private function getBulkDeliveryData($transactionDate, $itemCodes, $storeCodes, $storeSubUnitShortName)
    {
        $query = StoreReceivingInventoryItemModel::select([
            ...
            ->where('received_at', '>=', $transactionDate . ' 00:00:00')
            ->where('received_at', '<=', $transactionDate . ' 23:59:59');
    ...
    private function getBulkTransferData($transactionDate, $itemCodes, $storeCodes, $storeSubUnitShortName, $foodChargeReasonList)
    {
        $query = StockTransferModel::with([
            ...
            ->where('logistics_picked_up_at', '>=', $transactionDate . ' 00:00:00')
            ->where('logistics_picked_up_at', '<=', $transactionDate . ' 23:59:59');
    ...
    private function getBulkConversionData($transactionDate, $itemCodes, $storeCodes, $storeSubUnitShortName)
    {
        $query = StockConversionModel::with('stockConversionItems')
            ...
            ->where('created_at', '<=', $transactionDate . ' 23:59:59');
        ...
        foreach ($conversions as $conversion) {
            ...
            foreach ($conversion->stockConversionItems as $item) {
                ...
                // Sum quantities if the same item appears in multiple conversions
                if (!isset($result[$key]['convert_in'][$itemKey])) {
                    $result[$key]['convert_in'][$itemKey] = [
                        'item_code' => $item->item_code,
                        'quantity' => 0,
                    ];
                }
                $result[$key]['convert_in'][$itemKey]['quantity'] += $item->converted_quantity;
            }
        }
    ...
    private function getBulkActualCount($transactionDate, $itemCodes, $storeCodes, $storeSubUnitShortName)
    {
        ...
        // Build query to get the latest count per item-store combination
        $query = DB::table('stock_inventory_count as sic')
            ->join('stock_inventory_items_count as siic', 'sic.id', '=', 'siic.stock_inventory_count_id')
            ->join(
                DB::raw('(
                    SELECT
                        sic2.store_code,
                        sic2.store_sub_unit_short_name,
                        siic2.item_code,
                        MAX(sic2.created_at) as max_created_at
                    FROM stock_inventory_count sic2
                    JOIN stock_inventory_items_count siic2 ON sic2.id = siic2.stock_inventory_count_id
                    WHERE sic2.created_at >= "' . $transactionDate . ' 00:00:00"
                        AND sic2.created_at <= "' . $transactionDate . ' 23:59:59"
                        ' . ($storeSubUnitShortName ? 'AND sic2.store_sub_unit_short_name = "' . $storeSubUnitShortName . '"' : '') . '
                    GROUP BY sic2.store_code, sic2.store_sub_unit_short_name, siic2.item_code
                ) as latest'),
                function ($join) {
                    $join->on('sic.store_code', '=', 'latest.store_code')
                        ->on('sic.store_sub_unit_short_name', '=', 'latest.store_sub_unit_short_name')
                        ->on('siic.item_code', '=', 'latest.item_code')
                        ->on('sic.created_at', '=', 'latest.max_created_at');
                }
            )
            ->whereIn('sic.store_code', $storeCodes)
            ->whereIn('siic.item_code', $itemCodes)
            ->select(
                'sic.store_code',
                'sic.store_sub_unit_short_name',
                'siic.item_code',
                'siic.counted_quantity',
                'siic.remarks'
            );
